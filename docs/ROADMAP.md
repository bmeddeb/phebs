...
The declared product design target includes Bazel-managed Go monorepositories
with at least 5,000 logical services and source trees in the 12-GB class. The
exact dimensions are 5,000 simultaneously accepted service incarnations in
one exact repository/catalog generation and at least 12,000,000,000 declared Git
blob bytes across unique regular source paths admitted by one frozen HEAD
indexing profile, with generated, vendored, test, excluded, gitlink, and
external-repository treatment reported separately. The larger neutral
headroom target remains 10,000 accepted services over the
two-million-eligible-Go-path physical profile. Current per-file, per-member,
per-partition, per-generation, request, cache, and worker ceilings remain
...
Exact implementation `b6f80e94` closed its selected acceptance gate on
2026-09-12: the 74/25-name race groups produced 95 passes and four explicit
native opt-in skips; module verification, full static, docs and glossary checks
passed. Native archive/restore sampling and later-phase composition remain required
...
relationship build/projector/installed-reference and joined lifecycle observations are implemented,
...
race gates. T42.1r6 has closed F/Q native accounting, corrected physical R to
the production current/prior lifecycle with one exact
`41/0/4,063,208/0` serial report under a ten-minute, two-session exact-reader
admission, and bound logical activation residue/recovery to two exact
`0/10/0/0` snapshots with unchanged-search and exact scheduler-row fences; the
real controller call site observes the target only after commit and suppresses
...
T42.2f supplies remote accounting coordination through a second bounded
...
Exact source `676031a9749c7b484e3ba0bac64c9a606a654218` passed independent
zero-finding source/documentation/cost review, affected
...
focus without changing the URL or draft; and only **Discard edits and leave**
follows the captured exact hash. The browser-owned `beforeunload` warning,
...
- **Epic 41 — 10,000-service authority and sparse consumers (completed
  ...
  proof and stores precious root/member rows behind a dark pointer; T41.4 adds
  exact recovery/archive ownership and a fifteenth bounded lifecycle owner;
...
- T41.3 adds strict expanded-input ingestion, streamed source-census liveness
  ...
  path registers v3. Integration remains separate.
- T41.4 binds exact logical bytes, lifecycle metadata, and root/member edges
  into every dark publication; repairs only complete pre-lifecycle roots;
  validates the whole precious v3 inventory under live backup and after
  restore; and adds one shared-lock lifecycle owner with atomic tombstoning,
  candidate/state-reference/rollback protection, restartable member drains,
  malformed-row isolation, and exact byte status.
- T41.5 reuses generation-scheduler chunks and leases for distinct v3
  reconcile and activation plans. Reconcile withholds the matching summary
  until its final CAS; activation updates at most 512 rows plus that summary
  atomically. Successor publication fences reconcile and supersedes activation,
  terminal repair/continue is bounded, restore preserves precious state while
