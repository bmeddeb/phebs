...
| `make test`          | verify generated glossary projections, then `go test ./... -timeout=60m`; before running tests it fails if `surreal` is absent from `PATH`, and the runtime-only `PHEBS_SURREAL` override does not satisfy test discovery. Only exact `PHEBS_SKIP_SURREAL_TESTS=1` explicitly retains the engine-backed skips; any other nonempty value is rejected. The per-package timeout matches `ci-go` (whose enclosing CI job allows 90 minutes), and child tests build pinned zoekt and Buf binaries |
...
30-second request timeout. The complete two-profile suite has at most ten
endpoint checks because either relationship response may carry a citation. It
therefore permits at most 30 requests and 20 seconds of retry delay if each
...
drifted candidates. A descendant task-all-info `EPERM` adds one privilege-free
short BSD read only to prove that the PID is missing or no longer belongs to the
parent that discovered it; only that reuse/reparent transition is absent from
the sample. Root task-all-info `EPERM` is always sticky. A still-parented denied
child, invalid record, parent/class/identity
...
## T40.R1 neutral-39 source-free stop and V30

Retain neutral-39's sealed source-free package at digest
`sha256:681aef5bb4ebe77c63ed564f5dfe499609a76738c3172b7a58e9c9f87d6a43cb`
and record only that execution returned status 1. The last useful wrapper
monitor line reported `prepare/admission` and unknown custody age. Its later
terminal-summary `jq` expression failed because of shell quoting; repair that
monitor separately before reuse. This output did not disturb execution and
does not recover the private cause. Do not label neutral-39 `EPERM`, assign it
to phase 6 or any other phase, or infer a pipeline failure from unavailable
source-free evidence.

V30 keeps successful startup log inspection and health/stage observations when
the process sampler alone fails. Zero process counters in that projection mean
unavailable, not measured zero, and carry
...
`failed_phase_allocation_sampling_unavailable`. Mixed measurement failure keeps
`failed_phase_measurement_unavailable`. V1–V29 parsing, validation, and
outcome meanings remain exact.

For V30 warm restart, do not evaluate the paired startup/phase Git oracle at
HTTP health or the first successful reuse report alone. Within the existing
revalidation deadline, incrementally track phase-local candidate and job
lifecycle reports. Require a candidate reuse report whose decision is
`warm_noop`, `cold_reuse`, or `marker_recovery` and whose outcome is `done`;
reject released, failed, or requeued work. Treat claimed, started, deferred, and
yielded as unresolved persisted work; each observed job must later report
`event=done,outcome=success`. Re-inspect exact authority on every attempt. Once
all jobs resolve, require one existing five-second convergence interval with no
new candidate or job report. Finish the phase meter exactly once at that settled
boundary. Refresh paired startup process counters from that single finished
`PhaseMetrics` sampler snapshot—do not take a second process sample—and refresh
only log/stage/wall observations there. Settle the atomic warm→delta handoff of
the finished process window and exact log EOF with a bounded post-reset tail
...
are delta. Then require complete phase Git count to equal
the refreshed startup count. This bounded quiescence tolerates
health-before-sync/reuse but still rejects every sampled Git lifetime after the
boundary. V1–V29 keep their frozen warm behavior.

V30 ceremony servers set exact-control reporting for candidate output and the
sync, fetch, candidate, extraction, resolver, caller, and index job runners.
Each report is written synchronously. Any encode, size-cap, sink, or panic
failure latches cancellation; after background workers join, the server returns
a nonzero terminal error. Without exact-control mode, ordinary runtime sinks
remain advisory and never change queue or handler outcome.

For phase 7, remove ballast first and then establish the recovery fence. Accept
normal recovery only after all 14 owner attempts are newer than that fence in
one coherent sorted cycle and one final exact-normal capacity observation is
newer than the latest owner. Then re-inspect protected authority and require it
unchanged. Do not infer that capacity stayed normal throughout that cycle or
wait for the production runner to become hourly-idle eligible.

Independently, production idle recovery brackets the sorted cycle-start owner
...
differ. `CallerGeneration`, `RelationshipSemanticDigest`, and every other
product field must remain equal. The restored
readiness rehearsal uses the same comparator and requires all 14 owners fresh
and `state=ok`; an all-`not_run` status is not readiness. Require exact,
drained results for the 13 non-`durable-jobs` owners. Require `durable-jobs` to
remain truthfully `lower_bound`, permitting backlog while live writers exist,
and require exact capacity after the latest owner.

Phase 9 restarts the server to force a phase-local lifecycle cycle. Require the
sorted frozen 14-owner inventory to be fresh and report `state=ok`. Thirteen
non-`durable-jobs` owners must be exact with no backlog; `durable-jobs` must be
`lower_bound` and may retain backlog because live writers prevent an exact
oracle. Require exact capacity observed after the latest owner. Preserve
unchanged current stable authority and seal only the bounded per-owner
name/state/completeness/scanned/deleted/backlog projection.
Do not interpret nonzero `deleted` as a required live oracle and do not claim
this phase individually proves rollback, active-lease, marker, or store-pin
...
without a second process read, one bounded post-reset log-tail scan, and the
...
Focused Phase-7 result (2026-08-26): the separately run
`semantic-stale-worker` selector emitted `semantic stale-worker boundary
passed`; its subtest completed in 32.22 seconds, the top-level readiness test
in 197.17 seconds, and the package command in 197.835 seconds. Successful
cleanup removed that run's private workspace. The retained terminal record
does not bind an exact HEAD and clean-checkout proof, so exact candidate
attribution remains open. This is not prior-phase handoff, full-scale custody,
release, or SLO evidence.
...
Exact focused result (2026-08-26): the corrected wrapper subsequently passed
from clean commit `37fba2896f500104fa8283914ed19b8a003e3a24`. It emitted
`structural pressure collect/recovery boundary passed`; the subtest completed
in 252.48 seconds, the top-level readiness test in 310.35 seconds, and the
package command in 311.030 seconds. The correction commit is integrated into
`main`; a later read-only host check found no matching Phase-8 diagnostic path
or live rehearsal/Surreal process. This supersedes only the focused-rerun
...
Exact focused result (2026-08-26): the unchanged selector passed from clean
commit `15487bbf15b602b04d81fbae6b989777b5cac44d`. The Phase-10 subtest completed
in 147.89 seconds, the top-level readiness test in 205.45 seconds, and the
package command in 206.121 seconds. It emitted the required boundary marker,
removed its successful workspace, retained no diagnostic root, and left no
matching Phebs/Surreal process. That commit is integrated into and pushed as
`main`. This remains clean-entry focused evidence; the separately recorded
...
relationship, and citation oracles, exact control-read and two-meter accounting,
at least the three mandatory query-member reads, and shutdown. Existing unit
...
projection when present. Do not share or rerun against it; first prove all
...
Exact Phase-9 rerun (2026-08-26): the unchanged
`structural-archive-restore` selector passed under fixed-HEAD and
clean-worktree guards at commit
...
reruns. Cross-phase handoff, complete signed ceremony, release, closure, and
...
dedicated Darwin host. This is the supported entry path: it materializes and
verifies the committed wrapper in a private directory outside the checkout,
with the entire bootstrap under fixed `/usr/bin/env -i` and
`/bin/bash --noprofile --norc`.
Do not invoke the live worktree copy directly.
...
When `/private/tmp` is not the desired backing volume, add an explicit
`PHEBS_T4013_PHASE7_REPLAY_PARENT=/absolute/private/path` assignment to the
leading `/usr/bin/env -i` invocation. It must be outside the source
checkout. The wrapper prints the fresh run root before authoring. It binds the
canonical Go driver, clears ambient Go
workspace/overlay controls, and uses fresh private build and module caches. It
also binds Git, creates a fresh owner-only shared clone detached at the exact
commit with closed system/global config, attributes, hooks, excludes,
fsmonitor, and replacement objects, and compiles and runs only from that clone.
The Go child inherits the same closed Git environment for VCS stamping and
module fallback. The original checkout supplies Git
objects; the external bootstrap supplies the separately blob-verified wrapper,
and no unverified live source bytes become executable authority. The wrapper's
pre/post blob checks are later drift fences. It rejects modified, untracked, or ignored files in the private
source before and after execution, so ignored files and index flags in the live
checkout cannot change the replay binary.
...
its private FIFO. Any dead, extra, malformed, or uninspectable member retains
...
snapshot. Fail-closed quiescence permits at most 100 snapshots and roughly one
second of ten-millisecond waits per child. The sentinel alone holds the
notification writer while its workload runs with that descriptor closed, so
completion or hard death wakes the blocking parent read with a record or EOF
and launches no polling process. Exact job comparison adds one short
command-substitution Bash child at drain entry and one more only when a signal
handler enters. Clone, checkout, Go, and recursive
private-cache/source retirement make four such boundaries. Hydration and
compilation retain their already-documented lack of a wrapper deadline.
...
the run/control roots, exact source clone, and
`/private/tmp/phebs-t4013-phase7-full.lock`. Do not
...
and collection cycle and in lifecycle status. V1 keeps its frozen fourteen-owner
set and V2–V4 keep their sixteen-owner sets as history. The parent rejects an
old V4 launch at its historical sixteen-owner validation.
