...
| Pack version | `1.0.0` (`thrift-consumer` extractor) |
...
generated Go method name unique across the repository's stub index, wire
method name recovered from `processorMap` key literals). Operation objects
are `/scope.Service/method` with scope taken from the generated package
name, joining the declaration pack's D1 identity by string equality.
...
with candidate counts. Oversized and unparseable files produce
`THRIFT_EXTRACTION_GAP` (`source_too_large` / `parse_error`); empty files are
complete coverage. Underscored IDL method names camelize differently than
the upper-first index transform and therefore abstain rather than
misattribute (documented ceiling, D4).

**Validation.** T19.1 spike gate 3: zero false resolutions on the
hand-labeled jaeger-client-go sample, including the exact-identifier
negative (`NewAgentClientUDP`) and one measured handler-side heuristic-noise
emission (D6). Persisted-path parity: `TestThriftPackEndToEnd` drives both
packs through the real worker and store and requires the operation-object
join. These are rule-validation gates, not accuracy measurements.

**Operating envelope.** 4 MiB per Go source file; generated files are read
once for indexing and non-generated files once for scanning (within the
worker's read budget); worker bounds unchanged (5,000 facts, 15-minute
run).
