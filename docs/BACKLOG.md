...
T32.1 freezes the selected microservice-first program and validation matrix as
a documentation/decision ticket; T32.2 begins runtime measurement only after
the T31.1 merge bar closes. Epics 33–39 are dependency-ordered program drafts,
not implementation authorization. Epics 25–28 remain drafted and unscheduled;
none is an implicit next ticket.
...
Epics 0–24, Epic 29, Epic 30, and P5 hardening are retained in the
...
## Epic 32 · Microservice program contract and validation *(drafted 2026-08-03 · T32.1 decision work; runtime measurement needs T31.1)*
...
rename, split, merge, removal, inaccessible scope, and partial generation
states. Generate separate deterministic 1,000- and 5,000-service catalog/load
profiles after freezing their file/content/membership cardinalities. AC: two
runs produce byte-identical Git trees, catalog inputs, and oracles; the oracle
enumerates exact membership, all-code and per-service query results,
cross-service RPC/topic expectations, ambiguity, currentness, and tombstones
without consuming phebs output; public/generated bytes only; no production
registration.

**T32.4 · Search-topology and cost spike** *(needs T32.2, T32.3)* — compare
