...
| `experimental.provisional_thrift_field_extraction` | `false`   | independent development-only opt-in for T22's thriftrw field-reference reader over a committed root `index.scip`; no public query is added until T22.4             |
...
registers the T22.2 `scip-thrift-field` reader. It currently recognizes only
Go generated by thriftrw. An eligible generated file must embed a
...
Objects use `scope.Message#field-number`, including field `0`, and lineage
uses the same package-based `contract_scip_package_v1` family as protobuf
without claiming equality to the declaration pack's provisional lineage.
Duplicate definitions or duplicate `(message, field ID)` identities abstain.
...
accuracy, completeness, runtime-use, or absence claim. T22.2 adds ingestion
only: the existing `find_proto_field_references` route remains protobuf-only,
and the neutral Thrift-capable query/report/MCP surface is deferred to T22.4.
