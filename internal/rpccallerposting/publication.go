...
// soon as its immutable generation is installed.
func (prepared *Prepared) Discard() error {
	if prepared == nil || prepared.closed || prepared.directory == "" {
		return nil
	}
	base := publicationRepository(prepared.root, prepared.repository)
...
func (publication *Publication) ReadDigest(
	ctx context.Context,
	protocol, operation, digest string,
) (Posting, error) {
	...
	var err error
	if operation == "" {
		values, err = publication.ReadUnresolved(ctx, protocol)
	} else {
		values, err = publication.ReadOperation(ctx, protocol, operation)
	}
	if err != nil {
		return Posting{}, err
	}
