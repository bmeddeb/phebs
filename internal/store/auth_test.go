...
func TestAuthAPIKeysAndSessions(t *testing.T) {
	...
	_, err = s.CreateAPIKey(ctx, store.APIKey{
		ID: "key2", UserID: "user1", Name: "Investigation agent",
		Prefix: "phebs_key2", Hash: "second-digest",
		Capabilities: []store.APIKeyCapability{"investigation:write"},
	...
	for _, test := range []struct {
		...
		{"duplicate", []store.APIKeyCapability{"investigation:write", "investigation:write"}},
	} {
