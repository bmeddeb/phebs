...
var (
	errSCIPDocumentOutOfScope = errors.New("SCIP document is outside the committed analysis unit")
...
func (e extractor) Extract(
	...
	if err != nil {
		return coverage, fmt.Errorf("read SCIP input: %w", err)
	}
	if !input.Present {
		...
		if len(input.Content) == 0 {
			...
			var parseErr error
			documents, parseErr = parseIndexScoped(ctx, input.Content, corpus)
			if parseErr != nil {
				if errors.Is(parseErr, errSCIPDocumentOutOfScope) {
					return coverage, fmt.Errorf(
						"parse %s: %w", input.Path, parseErr,
					)
				}
				if err := emitGap(
					input.Path, input.Blob,
					"malformed_symbol_input", parseErr.Error(), emit,
				); err != nil {
					return coverage, err
				}
				...
				for _, document := range documents {
					...
					if err := emitGap(
						input.Path, input.Blob,
						"stale_symbol_input", document.path, emit,
					); err != nil {
						return coverage, err
					}
					coverage.UnresolvedCount = 1
...
func emitGap(
	...
	return emit(sdk.Fact{
		...
		Path: inputPath, StartLine: 1,
		EndLine: 1 + strings.Count(blob.Content[:len(blob.Content)-1], "\n"),
		Assertion: sdk.AssertionInput{
			Predicate: "CALLER_EXTRACTION_GAP", Subject: inputPath,
	...
	visitor := scip.IndexVisitor{
		...
		VisitDocument: func(ctx context.Context, document *scip.Document) error {
			...
			if scope, ok := corpus.(sdk.SCIPDocumentScope); ok &&
				!scope.SCIPDocumentInScope(document.GetRelativePath()) {
				return fmt.Errorf(
					"%w: %q",
					errSCIPDocumentOutOfScope, document.GetRelativePath(),
				)
			}
