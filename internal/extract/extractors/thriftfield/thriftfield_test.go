...
func TestApacheThriftTagParsing(t *testing.T) {
	rows := []struct {
		name, literal          string
		wantName, requiredness string
		wantID                 int
		wantFound, wantErr     bool
	}{
		{
			name: "default", literal: "`thrift:\"process,1\"`",
			wantName: "process", wantID: 1, requiredness: "default", wantFound: true,
		},
		{
			name: "required", literal: "`thrift:\"process,1,required\"`",
			wantName: "process", wantID: 1, requiredness: "required", wantFound: true,
		},
		{
			name: "optional field zero", literal: "`json:\"success\" thrift:\"success,0,optional\"`",
			wantName: "success", wantID: 0, requiredness: "optional", wantFound: true,
		},
		{name: "unrelated", literal: "`json:\"process\"`"},
		{name: "missing ID", literal: "`thrift:\"process\"`", wantErr: true},
		{name: "empty name", literal: "`thrift:\",1\"`", wantErr: true},
		{name: "non-numeric ID", literal: "`thrift:\"process,nope\"`", wantErr: true},
		{name: "negative ID", literal: "`thrift:\"process,-1\"`", wantErr: true},
		{name: "ID above bound", literal: "`thrift:\"process,32768\"`", wantErr: true},
		{name: "malformed literal", literal: "`thrift:\"process,1\"", wantErr: true},
	}
	for _, row := range rows {
		t.Run(row.name, func(t *testing.T) {
			name, id, requiredness, found, err := parseApacheThriftTag(row.literal)
			if row.wantErr {
				if err == nil {
					t.Fatalf("parseApacheThriftTag(%q) succeeded", row.literal)
				}
				return
			}
			if err != nil {
				t.Fatalf("parseApacheThriftTag(%q): %v", row.literal, err)
			}
			if name != row.wantName || id != row.wantID ||
				requiredness != row.requiredness || found != row.wantFound {
				t.Fatalf(
					"parseApacheThriftTag(%q) = %q, %d, %q, %v; want %q, %d, %q, %v",
					row.literal, name, id, requiredness, found,
					row.wantName, row.wantID, row.requiredness, row.wantFound,
				)
			}
		})
	}
...
func TestApacheReferencesAreDerivedAndFieldZeroIsFirstClass(t *testing.T) {
	generated := `// Code generated by Thrift Compiler (0.14.1). DO NOT EDIT.
package jaeger

type Batch struct {
	Process any ` + "`thrift:\"process,1,required\"`" + `
}

type Emit_Result struct {
	Success any ` + "`thrift:\"success,0,optional\"`" + `
}
`
	consumer := `package consumer

func use(batch *jaeger.Batch, result *jaeger.Emit_Result) {
	batch.Process = nil
	_ = result.Success
}
`
	processSymbol := "scip-go gomod example.com/apache v1.2.3 gen/Batch#Process."
	successSymbol := "scip-go gomod example.com/apache v1.2.3 gen/Emit_Result#Success."
	index := &scip.Index{
		Metadata: &scip.Metadata{},
		Documents: []*scip.Document{
			{
				RelativePath:     "thrift-gen/jaeger/jaeger.go",
				PositionEncoding: scip.PositionEncoding_UTF8CodeUnitOffsetFromLineStart,
				Occurrences: []*scip.Occurrence{
					{
						Range:  rangeFor(t, generated, "Process", 0, scip.PositionEncoding_UTF8CodeUnitOffsetFromLineStart),
						Symbol: processSymbol, SymbolRoles: int32(scip.SymbolRole_Definition | scip.SymbolRole_Generated),
					},
					{
						Range:  rangeFor(t, generated, "Success", 0, scip.PositionEncoding_UTF8CodeUnitOffsetFromLineStart),
						Symbol: successSymbol, SymbolRoles: int32(scip.SymbolRole_Definition | scip.SymbolRole_Generated),
					},
				},
			},
			{
				...
				Occurrences: []*scip.Occurrence{
					{
						Range:  rangeFor(t, consumer, "Process", 0, scip.PositionEncoding_UTF8CodeUnitOffsetFromLineStart),
						Symbol: processSymbol, SymbolRoles: int32(scip.SymbolRole_WriteAccess),
					},
					{
						Range:  rangeFor(t, consumer, "Success", 0, scip.PositionEncoding_UTF8CodeUnitOffsetFromLineStart),
						Symbol: successSymbol, SymbolRoles: int32(scip.SymbolRole_ReadAccess),
					},
				},
			},
		},
	}
	facts, coverage := extractFacts(t, memoryCorpus{
		repo: "example.invalid/apache", commit: strings.Repeat("3", 40),
		files: map[string]string{
			indexPath:                     encodeIndex(t, index),
			"thrift-gen/jaeger/jaeger.go": generated,
			"consumer/use.go":             consumer,
		},
	})
	if len(facts) != 2 {
		t.Fatalf("Apache facts = %d, want process and field-zero references: %+v", len(facts), facts)
	}
	if !reflect.DeepEqual(coverage.Protocols, []string{
		...
		t.Fatalf("Apache protocols = %v", coverage.Protocols)
	}
	byObject := make(map[string]sdk.Fact)
	for _, fact := range facts {
		byObject[fact.Assertion.Object] = fact
		if fact.Assertion.Tier != "derived" ||
			!strings.Contains(fact.Assertion.Detail, `"generator":"apache"`) ||
			!strings.Contains(fact.Assertion.Detail, `"source_binding":"none"`) {
			t.Fatalf("Apache assertion = %+v", fact.Assertion)
		}
		citation := consumer[fact.Atom.StartByte:fact.Atom.EndByte]
		if citation != "Process" && citation != "Success" {
			t.Fatalf("Apache citation = %q", citation)
		}
	}
	if process := byObject["jaeger.Batch#1"]; process.Assertion.Object == "" ||
		!strings.Contains(process.Assertion.Detail, `"classification":"write"`) {
		t.Fatalf("Apache process fact = %+v", process)
	}
	if success := byObject["jaeger.Emit_Result#0"]; success.Assertion.Object == "" ||
		!strings.Contains(success.Assertion.Detail, `"classification":"read"`) {
		t.Fatalf("Apache success fact = %+v", success)
	}
}

func TestApacheScopeDerivationAndTypedIdentityAbstentions(t *testing.T) {
	generated := `// Autogenerated by Thrift Compiler (0.9.3)
package jaeger
type Batch struct {
	Process any ` + "`thrift:\"process,1\"`" + `
	Untagged any
}
`
	cases := []struct {
		...
		{
			name:       "package supplies scope",
			source:     generated,
			symbol:     "scip-go gomod example.com/apache v1 gen/Batch#Process.",
			definition: "Process", wantObject: "jaeger.Batch#1",
		},
		{
			name:       "wrong enclosing type abstains",
			source:     generated,
			symbol:     "scip-go gomod example.com/apache v1 gen/Other#Process.",
			definition: "Process",
		},
		{
			name:       "untagged exact span abstains",
			source:     generated,
			symbol:     "scip-go gomod example.com/apache v1 gen/Batch#Untagged.",
			definition: "Untagged",
		},
		{
			name: "marker in string is ineligible",
			source: strings.Replace(
				generated,
				"// Autogenerated by Thrift Compiler (0.9.3)",
				`const marker = "// Autogenerated by Thrift Compiler"`,
				1,
			),
			symbol:     "scip-go gomod example.com/apache v1 gen/Batch#Process.",
			definition: "Process",
		...
		{
			name:       "header without tags is ineligible",
			source:     strings.Replace(generated, " `thrift:\"process,1\"`", "", 1),
			symbol:     "scip-go gomod example.com/apache v1 gen/Batch#Process.",
			definition: "Process",
		},
		{
			name:       "malformed field ID fails candidate",
			source:     strings.Replace(generated, "process,1", "process,nope", 1),
			symbol:     "scip-go gomod example.com/apache v1 gen/Batch#Process.",
			definition: "Process", wantErr: true,
		},
		{
			name:       "unusable package scope fails candidate",
			source:     strings.Replace(generated, "package jaeger", "package _", 1),
			symbol:     "scip-go gomod example.com/apache v1 gen/Batch#Process.",
			definition: "Process", wantErr: true,
		},
	}
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			consumer := "package consumer\nvar _ = batch." + test.definition + "\n"
			index := makeApacheIndex(
				t, test.source, consumer, test.definition, test.symbol,
				int32(scip.SymbolRole_ReadAccess),
			)
			corpus := memoryCorpus{
				repo: "example.invalid/apache", commit: strings.Repeat("4", 40),
				files: map[string]string{
					indexPath:         encodeIndex(t, index),
					"generated.go":    test.source,
					"consumer/use.go": consumer,
				},
			}
			var facts []sdk.Fact
			_, err := New().Extract(context.Background(), corpus, func(fact sdk.Fact) error {
				facts = append(facts, fact)
				return nil
			})
			if test.wantErr {
				if err == nil {
					t.Fatal("malformed Apache candidate succeeded")
				}
				return
			}
			if err != nil {
			...
			if test.wantObject == "" {
				if len(facts) != 0 {
					t.Fatalf("abstention emitted facts: %+v", facts)
				}
				return
			}
			if len(facts) != 1 || facts[0].Assertion.Object != test.wantObject {
				t.Fatalf("scope facts = %+v, want %q", facts, test.wantObject)
			}
		})
	}
}

func TestApacheDuplicateMessageFieldIDAbstains(t *testing.T) {
	generated := `// Code generated by Thrift Compiler (0.24.0). DO NOT EDIT.
package legacy
type Batch struct {
	First any ` + "`thrift:\"first,1\"`" + `
	Second any ` + "`thrift:\"second,1\"`" + `
}
`
	consumer := "package consumer\nvar _ = batch.First\nvar _ = batch.Second\n"
	first := "scip-go gomod example.com/legacy v1 gen/Batch#First."
	second := "scip-go gomod example.com/legacy v1 gen/Batch#Second."
	index := &scip.Index{
		Metadata: &scip.Metadata{},
		Documents: []*scip.Document{
			{
				...
				Occurrences: []*scip.Occurrence{
					{
						Range:  rangeFor(t, generated, "First", 0, scip.PositionEncoding_UTF8CodeUnitOffsetFromLineStart),
						Symbol: first, SymbolRoles: int32(scip.SymbolRole_Definition),
					},
					{
						Range:  rangeFor(t, generated, "Second", 0, scip.PositionEncoding_UTF8CodeUnitOffsetFromLineStart),
						Symbol: second, SymbolRoles: int32(scip.SymbolRole_Definition),
					},
				},
			},
			{
				...
				Occurrences: []*scip.Occurrence{
					{
						Range:  rangeFor(t, consumer, "First", 0, scip.PositionEncoding_UTF8CodeUnitOffsetFromLineStart),
						Symbol: first, SymbolRoles: int32(scip.SymbolRole_ReadAccess),
					},
					{
						Range:  rangeFor(t, consumer, "Second", 0, scip.PositionEncoding_UTF8CodeUnitOffsetFromLineStart),
						Symbol: second, SymbolRoles: int32(scip.SymbolRole_ReadAccess),
					},
				},
			},
		},
	...
	facts, _ := extractFacts(t, memoryCorpus{
		repo: "example.invalid/apache", commit: strings.Repeat("5", 40),
		files: map[string]string{
			indexPath:         encodeIndex(t, index),
	...
	if len(facts) != 0 {
		t.Fatalf("duplicate legacy.Batch#1 emitted facts: %+v", facts)
	}
}

func TestThriftRWAndApacheFamiliesCoexistInOneRepository(t *testing.T) {
	corpus := fixtureCorpus(t, "v1.0.0", "workflow_id", "WorkflowID", 10)
	index := decodeIndex(t, corpus.files[indexPath])
	apache := `// Code generated by Thrift Compiler (0.24.0). DO NOT EDIT.
package legacy
type Batch struct {
	Process any ` + "`thrift:\"process,1\"`" + `
}
`
	consumer := "package consumer\nvar _ = batch.Process\n"
	symbol := "scip-go gomod example.com/legacy v2.0.0 gen/Batch#Process."
	apacheIndex := makeApacheIndex(
		t, apache, consumer, "Process", symbol, int32(scip.SymbolRole_WriteAccess),
	)
	index.Documents = append(index.Documents, apacheIndex.Documents...)
	corpus.files[indexPath] = encodeIndex(t, index)
	corpus.files["generated.go"] = apache
	corpus.files["consumer/apache.go"] = consumer
	// makeApacheIndex uses consumer/use.go; point its reference document at the
	// coexistence fixture's distinct source path.
	index = decodeIndex(t, corpus.files[indexPath])
	index.Documents[len(index.Documents)-1].RelativePath = "consumer/apache.go"
	corpus.files[indexPath] = encodeIndex(t, index)

	facts, _ := extractFacts(t, corpus)
	var exact, derived int
	for _, fact := range facts {
		switch fact.Assertion.Tier {
		case "exact":
			exact++
			if !strings.Contains(fact.Assertion.Detail, `"generator":"thriftrw"`) {
				t.Fatalf("exact family detail = %s", fact.Assertion.Detail)
			}
		case "derived":
			derived++
			if fact.Assertion.Object != "legacy.Batch#1" ||
				!strings.Contains(fact.Assertion.Detail, `"generator":"apache"`) {
				t.Fatalf("derived family fact = %+v", fact)
			}
		}
	}
	if exact != 2 || derived != 1 {
		t.Fatalf("coexistence tiers = exact:%d derived:%d; facts=%+v", exact, derived, facts)
	}
}
...
func makeApacheIndex(
	t *testing.T,
	generated, consumer, identifier, symbol string,
	referenceRoles int32,
) *scip.Index {
	t.Helper()
	return &scip.Index{
		Metadata: &scip.Metadata{},
		Documents: []*scip.Document{
			{
				...
				Occurrences: []*scip.Occurrence{
					{
						Range: rangeFor(
							t, generated, identifier, 0,
							scip.PositionEncoding_UTF8CodeUnitOffsetFromLineStart,
						),
						Symbol: symbol, SymbolRoles: int32(scip.SymbolRole_Definition | scip.SymbolRole_Generated),
					},
				},
			},
			{
				...
				Occurrences: []*scip.Occurrence{
					{
						Range: rangeFor(
							t, consumer, identifier, 0,
							scip.PositionEncoding_UTF8CodeUnitOffsetFromLineStart,
						),
						Symbol: symbol, SymbolRoles: referenceRoles,
					},
				},
			},
		},
