...
func TestParsedBlobProductionRefusalSticky(t *testing.T) {
	for _, mode := range []string{"missing", "sink", "panic", "canceled", "during"} {
		t.Run(mode, func(t *testing.T) {
			...
			if mode != "missing" {
				var err error
				ctx, err = readaccounting.WithParsedBlobObserver(ctx, func() error {
					switch mode {
					case "panic":
						panic("observer")
					case "during":
			...
			if err := ObserveProductionParsedBlob(ctx); err == nil {
