...
func TestT422PublicationFraming(t *testing.T) {
	...
	header, err := t422PublicationBinding(initial)
	if err != nil || string(header) != "EPB1:2:sha256:01"+strings.Repeat("00", 31)+"\n" || len(header) != 79 {
	...
	for _, mode := range []string{"valid", "phase", "producer", "input", "mode"} {
		t.Run(mode, func(t *testing.T) {
			...
			raw, err := t422PublicationRecord(state, initial)
			if (err == nil) != (mode == "valid") || err == nil && string(raw[:]) != "EP1:2:2\n" {
				t.Fatal(raw, err)
			}
		})
	}
	// The ordinary entry returns before installing any selected-family sink.
