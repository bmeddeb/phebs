...
| 2026-07-10 | T6.3 live PAT verification | GitHub adapter run end-to-end against a real PAT (200-repo account: `users:` 2-page pagination, `orgs:`, `repos:`, ~195 exclude globs → exactly the 5 kept repos synced/indexed/searchable, 4 of them private). **Finding fixed: `/users/{name}/repos` never returns private repos, even authenticated** — a `users:` entry naming the token's own login now resolves `GET /user` once and lists via `/user/repos` (case-insensitive login match; regression test with a fake API). Verified live: private repo reached the index *through the users listing*; token absent from mirror config, data dir, and API responses. Rate-limit wait not trippable live (5k/hr headroom) — stays covered by unit tests |
