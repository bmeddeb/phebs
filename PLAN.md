...
| 2026-07-11 | T8.2+T8.3 MCP server | Official `modelcontextprotocol/go-sdk` (locked in §1), mounted at **`/api/mcp` on the existing mux** — Streamable HTTP, same port, same `auth.api_key` bearer (plain-HTTP `RequireBearer` wrapper; the huma middleware only guards huma ops). Three typed tools (`search_code`, `read_file`, `list_repos`); schemas inferred from Go types. `read_file` is ranged (`start_line`/`end_line`) and caps output at 200 KB with a `truncated` flag (whole lines while they fit; a single over-cap line is byte-truncated UTF-8-safely so progress is guaranteed) — agent context is the scarce resource, not bandwidth; binary files and over-10 MiB blobs are tool errors, not base64. **AC verified live**: headless Claude Code session (`--mcp-config`, http transport) listed → searched → read a fixture needle correctly; unauthenticated requests 401 |
