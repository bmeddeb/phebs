...
func FrozenHostPlan(ctx context.Context, sourceCommit string) (Plan, error) {
	...
	return freshV32PlanWithHostToolchain(sourceCommit, hostToolchain, time.Now())
}
...
func frozenV26PlanWithHostToolchain(sourceCommit string, hostToolchain []HostToolObservation) (Plan, error) {
	value, err := frozenV25PlanWithHostToolchain(sourceCommit, hostToolchain)
	if err != nil {
		return Plan{}, err
	}
	value.Schema = PlanSchemaV26
	if err := ValidatePlan(value); err != nil {
		return Plan{}, err
	}
	return value, nil
...
//nolint:unused // Retained for exact historical V27 plan reconstruction.
...
func frozenV28PlanWithHostToolchain(sourceCommit string, hostToolchain []HostToolObservation) (Plan, error) {
	value, err := frozenV27PlanWithHostToolchain(sourceCommit, hostToolchain)
	if err != nil {
		return Plan{}, err
	}
	value.Schema = PlanSchemaV28
	if err := ValidatePlan(value); err != nil {
		return Plan{}, err
	}
	return value, nil
...
//nolint:unused // Retained for exact historical V28 plan reconstruction.
func freshV28PlanWithHostToolchain(
	sourceCommit string,
	hostToolchain []HostToolObservation,
	frozenAt time.Time,
) (Plan, error) {
	return freshPlan(frozenV28PlanWithHostToolchain, sourceCommit, hostToolchain, frozenAt)
}

func frozenV29PlanWithHostToolchain(sourceCommit string, hostToolchain []HostToolObservation) (Plan, error) {
	value, err := frozenV28PlanWithHostToolchain(sourceCommit, hostToolchain)
	if err != nil {
		return Plan{}, err
	}
	value.Schema = PlanSchemaV29
	if err := ValidatePlan(value); err != nil {
		return Plan{}, err
	}
	return value, nil
...
//nolint:unused // Retained for exact historical V29 plan reconstruction.
func freshV29PlanWithHostToolchain(
	sourceCommit string,
	hostToolchain []HostToolObservation,
	frozenAt time.Time,
) (Plan, error) {
	return freshPlan(frozenV29PlanWithHostToolchain, sourceCommit, hostToolchain, frozenAt)
}

func frozenV30PlanWithHostToolchain(sourceCommit string, hostToolchain []HostToolObservation) (Plan, error) {
	value, err := frozenV29PlanWithHostToolchain(sourceCommit, hostToolchain)
	if err != nil {
		return Plan{}, err
	}
	value.Schema = PlanSchemaV30
	if err := ValidatePlan(value); err != nil {
		return Plan{}, err
	}
	return value, nil
...
func frozenV32PlanWithHostToolchain(sourceCommit string, hostToolchain []HostToolObservation) (Plan, error) {
	value, err := frozenV31PlanWithHostToolchain(sourceCommit, hostToolchain)
	if err != nil {
		return Plan{}, err
	}
	value.Schema = PlanSchemaV32
	if err := ValidatePlan(value); err != nil {
		return Plan{}, err
	}
	return value, nil
...
func freshV32PlanWithHostToolchain(
	sourceCommit string,
	hostToolchain []HostToolObservation,
	frozenAt time.Time,
) (Plan, error) {
	return freshPlan(frozenV32PlanWithHostToolchain, sourceCommit, hostToolchain, frozenAt)
}
