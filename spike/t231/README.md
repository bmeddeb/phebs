...
| KD6 | Literal-or-abstain: a string literal or a **same-file `const`** resolves (`binding: literal` / `same-file-const`); everything else abstains with a frozen shape class — `selector-expr`, `call-expr`, `unresolved-ident`, `non-literal-expr`, `invalid-topic-literal`, `ambiguous-library-import`. A file importing both Sarama eras cannot truthfully attribute an untyped Consume receiver and therefore abstains without naming either import. A same-file `var` reports `unresolved-ident`. Every row carries an exact source-expression byte/line span and an explicit tier. No dataflow, no cross-file resolution, no constant propagation. |
