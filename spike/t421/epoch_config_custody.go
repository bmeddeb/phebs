...
type ExecutionEpochConfig struct {
	...
	SearchWarm             string // V5 restored epoch five only: native phase-14 search warm opt-in.
}
...
func PrepareExecutionEpochConfigs(ctx context.Context, author *ExecutionAuthorCustody) (_ *ExecutionEpochConfigCustody, retErr error) {
	...
	var parsedConfigs [5]*config.Config
	for index, logical := range []string{"a", "b", "a-return", "a-return", "a-return"} {
		...
		raw, parsed, err := epochConfigBytesParsed(plan, epoch, source)
		if err != nil {
			return custody, ErrExecutionEpochConfigs
		}
		epoch.ConfigSHA256 = SHA256(raw)
		custody.epochs[index] = epoch
		parsedConfigs[index] = parsed
...
// The parsed value comes from the actual generated bytes, not from a frozen
// profile projection. The constructor retains it only after copy protection.
func epochConfigBytesParsed(plan Plan, epoch ExecutionEpochConfig, source string) ([]byte, *config.Config, error) {
	...
	if logical < 0 || epoch.Epoch < 1 || epoch.Epoch > 5 ||
		...
		return nil, nil, ErrExecutionEpochConfigs
	}
	...
	if err != nil || len(raw) > maxEpochConfigBytes {
		return nil, nil, ErrExecutionEpochConfigs
	}
	parsed, err := parseEpochConfigBytes(plan, epoch, source, raw)
	if err != nil {
		return nil, nil, ErrExecutionEpochConfigs
	}
...
func parseEpochConfigBytes(plan Plan, epoch ExecutionEpochConfig, source string, raw []byte) (*config.Config, error) {
