...
func lifecycleTestBindings(producer uint32) string {
	...
	return fmt.Sprintf("ATB1:%d:%sSRB1:%d:%sOPB1:%d:%sLCB1:%d:%sCCB1:%d:%sEPB1:%d:%s", producer, input, producer, input, producer, input, producer, input, producer, input, producer, input)
}
