...
func TestExecutionEpochOneOptionalRealStartRehearsal(t *testing.T) {
	...
	for _, role := range []string{"t422-author", "phebs", "zoekt-git-index", "buf", "phebs-focused-index"} {
		selected := productionRehearsalBuildSchema(t, ctx, inputs, workspace, role, activeExecutionPlanSchema)
		started = time.Now()
		tool, err := inputs.protectReferenceTool(ctx, parent, role, selected, activeExecutionPlanSchema)
		if tool != nil {
	...
	plan, err := buildActiveExecutionPlan(commit)
