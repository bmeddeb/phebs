...
func applyExecutionCorrection(plan *Plan) error {
	...
	plan.Correction = &ContractCorrection{
		...
		ReadAccountingPolicy:      "v2-scope=H/X/T/F/L/R/Q/prep;not-total-pipeline-I/O;C=file-control-attempt;S=read-query-attempt;K=C+S;W=enqueue-transaction-attempt;meta=0;" + correctedInspectionPolicy + ";T-C=4;T-S=4;T-M=0;T-W=0;R:p=1xC(17+1+3+17+3=41)S0M(2*physical.combined_physical_owners)W0;l=2xC0S(2*(selector+plan+schedule+unit+selector-confirm)=10)M0W0;r2C5;s2C4S4;p2C7S4;80/90/75/lc=2/1/3/1xC0S0M0W0;ar=1xC1S0M0W0;Q-C=2*(2+2*16+6*5+8*2)=160;Q-S=2*(2+1+7+2*11+6*4+8*3)+3+1=164;Q-M=checked-plan-sum(query_results.results[*].(http+mcp).member_reads);Q-W=0;prep-C=24+4D+N+checkpoint+cold-open+[0,1]-binding-reread;prep-S=D+10+sum(A1..A4),Ai=[1,64];phase-K=prep+X+T+F+L+R+Q",
