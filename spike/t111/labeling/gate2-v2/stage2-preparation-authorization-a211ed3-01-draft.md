...
- Canonical candidate size: `10,355` bytes
- Canonical candidate SHA-256:
  `sha256:c3ccedd495e3cd23dcc35e2b2f074df9fdf14d781eef5dfde1b91080ff1721cf`
...
- Python mode: `isolated-no-site` (`-I -S -B`)
...
The accepted synthetic suite passes 19/19 with exit 0 under the resolved
Python 3.9.6 runtime and the exact `-I -S -B` mode; it does not access sealed
preparation inputs.
...
`-I` implies `-E` under the bound Python 3.9.6 runtime, so the interpreter
ignores `PYTHONPATH`, `PYTHONHOME`, `PYTHONSTARTUP`, and other `PYTHON*`
configuration variables. It does not erase variables from `os.environ`; the
exact empty-except-for-four launch environment above separately requires the
three named variables to be absent. `-S` suppresses implicit `site` loading,
and `-B` prevents bytecode-file writes.
...
3. In the same promotion commit, add the sole strict PLAN row:
   ...
   `| <date> | GATE2-V2 Stage-2 preparation authorization | AUTHORIZATION: APPROVED | authorization_id=t111-gate2-v2-prep-a211ed3-01;authorization_sha256=sha256:c3ccedd495e3cd23dcc35e2b2f074df9fdf14d781eef5dfde1b91080ff1721cf |`
...
- [ ] Embedded 10,355-byte candidate reserializes byte-identically and hashes
      to `sha256:c3ccedd495e3cd23dcc35e2b2f074df9fdf14d781eef5dfde1b91080ff1721cf`
